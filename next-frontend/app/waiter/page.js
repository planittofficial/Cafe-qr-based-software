...
export default function WaiterPage() {
  ...
  const downloadReceiptPdf = (order) => {
    ...
    const hasLaterAdditions =
      Boolean(order.updatedAt && order.createdAt) &&
      new Date(order.updatedAt).getTime() - new Date(order.createdAt).getTime() > 60 * 1000;
    ...
    const html = `
      ...
      <html>
        ...
        <body>
          ...
          ${hasLaterAdditions ? `
            <div class="note-box">
              <div class="note-title">Order activity</div>
              <div class="note-text">This bill includes items added later by the same guest on this table.</div>
            </div>
          ` : ""}
